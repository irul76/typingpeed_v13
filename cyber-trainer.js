...
// ================================================================
// INJECT PAGE HTML — isi konten ke page-cybertrainer yang sudah ada di index.html
// ================================================================
function injectCyberPage() {
  const page = document.getElementById('page-cybertrainer');
  if (!page) { console.error('CyberTrainer: #page-cybertrainer not found di index.html'); return; }
  // Jika sudah diisi (bukan loading placeholder), skip
...
// ================================================================
// AUTO INIT — dipanggil oleh navigate() di index.html
// ================================================================
// cyInit() sudah di-hook langsung di navigate() index.html:
//   if(page==='cybertrainer'){setTimeout(()=>{ if(typeof cyInit==='function') cyInit(); },100);}

document.addEventListener('DOMContentLoaded', () => {
  // 1. Inject CSS dulu
  injectCyberCSS();

  // 2. Inject char styles untuk typing display
  if (!document.getElementById('cy-char-styles')) {
    ...
    s.textContent = `
      ...
      .char.wrong    { color:#f76a6a; background:rgba(247,106,106,0.15); border-radius:2px; }
  ...
  // 3. Inject HTML page ke .main
  injectCyberPage();

  // 4. Jika halaman cybertrainer sudah active saat load
  if (document.getElementById('page-cybertrainer') &&
      document.getElementById('page-cybertrainer').classList.contains('active')) {
    cyInit();
  }
});
